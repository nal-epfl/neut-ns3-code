...
//
// - All links are P2P
// - Wehe applications runs along d0->s0, d1->s0 back-to-back
...
/* ################################################ READ AND PREPARE PARAMETERS (START) ################################################ */
    ...
    double lambda = 0.001;                          // in case of posssion/constant application: the inter-arrival time between probs
    ...
    cmd.AddValue("controlTestDuration", "duration to run the control measurements", controlTestDuration);
    cmd.AddValue("suspectedTestDuration", "duration to run the measurements that may be policed", suspectedTestDuration);
    ...
    cmd.AddValue("lambda", "in case of posssion/constant application: the inter-arrival time between probs", lambda);
...
/* ################################################ BUILD AND CONFIGURE TOPOLOGY (START) ################################################ */
    ...
    if (nonCommonLinksDelaysStr != "empty") { nonCommonLinkDelays = HelperMethods::SplitStr(nonCommonLinksDelaysStr, ','); }

    vector<string> nonCommonLinkRates(nbServers, defaultNonCommonLinkRate);
    if (nonCommonLinksDataRatesStr != "empty") { nonCommonLinkRates = HelperMethods::SplitStr(nonCommonLinksDataRatesStr, ','); }
    ...
    for (uint32_t i = 0; i < nbServers; i++) {
        ...
        if (HelperMethods::doesPolicerLocationMatch("nc"+to_string(i), policerLocation)) {
    ...
    if (HelperMethods::doesPolicerLocationMatch("c", policerLocation)) {
        TrafficControlHelper policerTch = CbQueueDisc::GenerateDisc1FifoNPolicers(
...
/* ########################################### SETUP THE APPLICATIONS AND BACKGROUND (START) ########################################### */
    ...
    if (HelperMethods::isPolicerTypePerFlowPolicer(policerType)) { trafficClass[nbApps - 1] = 8; }
    ...
    for (uint32_t i = 0; i < nbApps; i++) {
        ...
        if (appType == 5) {
            ...
            appsKey.emplace_back(HelperMethods::GetNodeIP(appsServer[i], 1), clientIP, weheCS->GetPort(), 0);
            continue;
        }
    ...
    for (uint32_t i = 0; i < nbServers; i++) {
        ...
        double throttledProb = HelperMethods::isPolicerTypePerFlowPolicer(policerType) ? 0 : throttlingPctOfBack;
...
//    p2pRouters.EnablePcap(resultsPath + "/router-link", 0, 1);
    ...
    vector<PacketMonitor *> nonCommonLinksMonitors;
    for (uint32_t i = 0; i < nbApps; i++) {
    ...
    for (uint32_t i = 0; i < nbApps; i++) {
        ...
        nonCommonLinksMonitors[i]->SaveRecordedPacketsCompact(resultsPath + "/app" + to_string(i) + "_noncommon_link_packets.csv");
    }
