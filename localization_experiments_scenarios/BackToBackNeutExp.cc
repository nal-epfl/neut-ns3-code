...
//
// - All links are P2P
// - Measurement applications runs along d0->s0, d1->s0 back-to-back
...
//    p2pRouters.EnablePcap(resultsPath + "/router-link", 0, 1);
    ...
    for (uint32_t i = 0; i < nbApps; i++) {
        auto *pathMonitor = new PacketMonitor(
                testsStartTime[testId[i]], testsEndTime[testId[i]], appsServer[i], client,"path_" + appsTag[i]);
        pathMonitor->AddAppKey(appsKey[i]);
        appsMonitors.push_back(pathMonitor);

        auto *commonLinkMonitor = new PacketMonitor(
                testsStartTime[testId[i]], testsEndTime[testId[i]], routerR, client, "commonLink_" + appsTag[i]);
        ...
        auto *nonCommonLinkMonitor = new PacketMonitor(
                testsStartTime[testId[i]], testsEndTime[testId[i]], appsServer[i], routerR, "noncommonLink_" + appsTag[i]);
