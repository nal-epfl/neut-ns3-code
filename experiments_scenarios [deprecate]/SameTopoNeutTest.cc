...
/* ################################################ READ AND PREPARE PARAMETERS (START) ################################################ */
    ...
    float controlTestDuration = 120.; // for how long measurement traffic should run for control measurements
    float suspectedTestDuration = 120.; // for how long measurement traffic should run for measurements that can policed
    ...
    string nonCommonLinksDelaysStr = "empty"; // you can pass the propagation delays of the non-common links
    string nonCommonLinksDataRatesStr = "empty"; // you can pass the link bandwidth of the non-common links

    CommandLine cmd;
    cmd.AddValue("linkRate", "the rate of the common link", commonLinkRate);
    cmd.AddValue("controlTestDuration", "the duration of the the simulation for control test", controlTestDuration);
    cmd.AddValue("suspectedTestDuration", "the duration of the the simulation for suspected test", suspectedTestDuration);
    ...
    cmd.AddValue("nonCommonlinksDelays", "the propagation delay of the 5 non-common links", nonCommonLinksDelaysStr);
    cmd.AddValue("nonCommonlinksDataRates", "the bandwidth of the 5 non-common links", nonCommonLinksDataRatesStr);
    cmd.Parse(argc, argv);
